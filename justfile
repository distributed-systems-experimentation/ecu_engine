...
MESSAGE_CRATE_VERSION := `cd ecu_engine_messages && cargo pkgid | cut -d "#" -f2`

publish-message-crate:
	cd ecu_engine_messages && cargo semver-checks --baseline-rev `git describe --tags --match 'messages-v*' --abbrev=0` && cargo publish
...
publish-image:
	docker push localhost:5432/ecu_engine:{{CRATE_VERSION}}
